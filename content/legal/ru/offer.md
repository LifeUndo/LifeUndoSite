...
5.1. ПО работает локально; телеметрия не ведётся, контент таймлайна и история браузера не передаются на сервер. На сервере {{COMPANY_SITE}} обрабатываются учётные данные (e-mail), идентификаторы устройств и статус лицензии, подписки и Cloud/TEAM, а также минимальные платёжные данные, необходимые платёжным провайдером.  
5.2. Дополнительные условия обработки персональных данных, включая централизованный учёт лицензий, подписок и Cloud/TEAM, содержатся в Политике обработки персональных данных: {{COMPANY_SITE}}/ru/legal/pdp.
